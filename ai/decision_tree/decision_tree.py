...
class DecisionTree(object):
    ...
    def training(self, training_data):
        ...
        y_index = c_idx[-1]
        self.build_tree(root, key, training_data, c_idx, r_idx, y_index,
                        self.column_names)
        return root

    @classmethod
    def build_tree(cls, tree, key, training_data, c_idx, r_idx, y_index,
                   column_names):
        # start training
        row_cnt = len(r_idx)
        ...
        # no t, i must determine the classification
        if not t_list:
            # change parent tag, make parent node as a leaf node
            tag = "?"
            current_max_p = 0
            for y, n in _v_y["ref_self"].items():
                p = float(n) / row_cnt
        ...
        for k in keys:
            ...
            cls.build_tree(tree[key], k, training_data, new_c_idx, new_r_idx,
                           y_index, column_names)
